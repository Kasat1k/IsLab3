...
class Group:
    def __init__(self, group_name, student_amount, subgroups):
        self.number = group_name
...
class Schedule:
    def __init__(self):
        ...
        self.quality = None  # Буде розраховано
...
def selection(population):
    ...
    population.sort(key=lambda x: x.quality, reverse=True)
...
def genetic_algorithm():
    ...
    for generation in range(GENERATIONS):
        ...
        best_quality = max(schedule.quality for schedule in population)
        if (generation + 1) % 10 == 0 or best_quality == 1.0:
            print(f'Generation {generation + 1}: Best quality = {best_quality}\n')
        if best_quality == 1.0:
            break
    best_schedule = max(population, key=lambda x: x.quality)
